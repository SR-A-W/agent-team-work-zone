...
description: >
  ...
  为每个 active teammate 用 Agent 工具 spawn 新 session（传 team_name + name），
...
- **`/reactivate-team <name>`——单独唤回一个指定 teammate**（典型用于把一个 `benched` 的唤回，也可强制单独重建某个 active）：只 spawn 这一个 name（Step 3）。成功后若它原本是 `benched`，改回 `active` 并清掉 `benched_at` / `bench_reason`（详见 Step 4）。
  - **关于 Step 0（重要）**：若本 session 里 team 注册**已存在**（还有其他 active teammate、或本 session 已跑过无参 reactivate）——**跳过 Step 0 的 `TeamDelete` / `TeamCreate`**，直接 spawn。否则 `TeamDelete` 会清掉正活着的其他 teammate 的注册、`TeamCreate` 也会撞"team 已存在"。仅当本 session **尚无任何 team 注册**（如刚重启就直接点名唤回、且没有别的 active teammate）时，才先 `TeamCreate`（必要时 best-effort `TeamDelete`）。
...
1. **`config.json` 里有该 member 条目** → ✗  
   `config.json` **磁盘文件持久存在**、且 members 数组永久残留死亡进程的 ghost 条目（`tmuxPaneId:"in-process"` 永不更新）——它记录的是历史注册，不反映任何进程是否存活。（注：真正 session-bound 的是 **runtime 内存注册**，不是这个磁盘文件——这也是 Step 0 为什么要"先 `TeamDelete` 清磁盘僵尸、再 `TeamCreate` 重建 runtime 注册"。详见 developer_manual「1. 两层 team 状态」段。）
...
### Step 0: 重建 team 注册（先清僵尸，再重建；执行时机：Step 2 用户确认之后、Step 3 spawn 之前）

**为什么必须有这一步**（两层模型，改这个 skill 前必读）：

Claude Code 的 team 状态分两层，跨 session 行为**不同**，不处理任一层都会让 reactivate 失败：

| 层 | 跨 session | 不处理的后果 |
|---|---|---|
| runtime 内存 team 注册 | ❌ session-bound，`/resume` **不**恢复 | 后续任何 `Agent(team_name=…)` spawn 直接报 `Team "X" does not exist. Call spawnTeam first`——全部 teammate spawn 失败 |
| 磁盘 `~/.claude/teams/<team>/config.json` | ✅ **持久**（文件留磁盘，且保留死 in-process 成员的 ghost 条目）| 用原名 spawn 撞上残留 ghost → Claude Code 自动加 `-2/-3` 后缀（实测行为）→ 名字漂移、`SendMessage` 找不到人 |

这两层**同源**（team 注册跨 session 不一致），用**同一步**根治：先 `TeamDelete` 清残留 ghost，再 `TeamCreate` 重建干净注册。

> ghost 为什么不可信、为什么要清，见本技能开头「前提：如何判断 teammate 是否存活」段陷阱 #1。

**执行时机**：本步编号为 Step 0（概念上是 spawn 的前置），但**实际执行**放在 Step 1（已读到确有 active teammate）+ Step 2（用户确认）**之后**、Step 3 spawn 循环**之前**——这样若 Step 1 早退（无 teammate 可恢复），就不会为一个空 team 白建注册。核心约束不变：**先于任何 spawn、且不影响 Step 3.1/3.3 的回执判定**。

**动作**（在任何 spawn 之前执行一次）：

1. 调 `TeamDelete`（**无参**工具，作用于"当前 session 的 team 上下文"）。
   - 它是**尽力而为的清场**：刚重启的 lead 通常没有 team 上下文，`TeamDelete` 会优雅 no-op（返回 `{"success":true,"message":"No team name found, nothing to clean up"}`）——**这是正常的，不是错误**。
   - 若报 "no team" / "active members" / "team 不存在" 等 → **一律吞掉、继续**（本来就是要它不存在/被清）。**不要**因 `TeamDelete` 的任何返回而中止流程。
2. 调 `TeamCreate(team_name="<your_team>")` 重建注册。**这一步是 load-bearing**：它让后续 spawn 不再报 "Team does not exist"；且因注册是新建的、干净的，spawn 能拿回原名、不带 `-2` 后缀。
   - 仅当 `TeamCreate` **本身**失败（极少见）→ 这是比 Step 3.3 回执判定**更早**的失败模式：直接报错给用户、**中止** reactivate，不进入 Step 3 spawn 循环。不要和 Step 3.3 的"回执判定"混为一谈。

> 实测补充：`TeamDelete` 主要在"**同一 session 内重复 reactivate**"（config.json 在本 session 内已累积 ghost）时才真正清到东西；**纯重启**场景下 runtime 注册本就失效、磁盘 ghost 也会被 `TeamCreate` 的新注册覆盖，`TeamCreate` 一步即干净。两种情况都"先 Delete 再 Create"，无需自己区分。
...
Agent(
    ...
    team_name="<team_name>",                    # 关键：让它真正加入 team
    name="<teammate-name>",                     # 关键：它在 team 里的 name
...
### Ghost collision（死成员名字残留）——已由 Step 0 内置根治

`~/.claude/teams/<team_name>/config.json` 的 `members` 数组会残留上次 session 的死 in-process 成员（ghost）条目。**实测行为**（不再是 future work）：用原名 spawn 时 Claude Code **自动加后缀**——`Fixer-2`，下次 `-3`、`-4`，每次 reactivate 累积；**不是**拒绝、也**不是**覆盖。副作用：名字漂移，`SendMessage to: Fixer` 会失败（它现在叫 `Fixer-2`），TEAMMATE_INFO.json 的 name 也得跟着改。

**本 skill 已内置根治**：Step 0 的 `TeamDelete` → `TeamCreate` 在 spawn 之前先清掉残留 ghost、重建干净注册，所以正常流程下 spawn 能拿回原名、不会出现 `-2`。

> 设计决策（已拍板，2026-05~06）：**不**单独做 `/cleanup-stale-members` skill、也**不**做 `--clean` 开关。清理与"重建注册"（Step 0 治 `Team does not exist`）**同源**，内聚成同一步即可，单拉一个 skill 是该聚不聚。

退化情况：万一某次 `TeamDelete` 没清干净、spawn 仍被加后缀 → 把该 teammate 标 `failed_to_reactivate` 告知用户；或接受 `-2` 名字并**同步更新 TEAMMATE_INFO.json 里该成员的 name**（否则后续 `SendMessage` 找不到它）。
