...
详细机制说明见 `developer_manual.md` 的「跨会话持久化机制限制」段。
...
```
用户: Architect, 我启动了 Qwen2.5-72B 的 SFT，想盯着点
...
Architect: 已 spawn tracker teammate（pane %12）。watchlist:
          squeue + ./runs/sft_qwen72b/status + train.log 尾部。
...
- `developer_manual.md` — 如何新增 skill / agent / 原型；源规则；发布流程
- `agent-teams.md` — 新架构的设计文档（why & how）
- `design_history.md` — 本次重构的决策历史
- `roadmap/autonomous_team_mode.md` — Terminal form (自主模式) 预留方案
